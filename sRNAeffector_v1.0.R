...
option_list = list(
		     ...
		     make_option(c("-d", "--miRdb"),type="character", default="0", help="Database of high confidence miRNAs. If using option either input 'miRBase' or 'MirGeneDB'.", metavar="character" )
		       ) 
...
cat(paste("\n\n Generating CDF plot for", miRinput))
        miRinput <- toupper(gsub("miR-","MIR",miRinput))
	miRinput <- toupper(gsub("LET-","LET_",miRinput))
	miRinput <- toupper(gsub("-","_",miRinput))
...
if(opt$miRdb == "MirGeneDB"){
mir_lvl <- read.table(paste0(sewd,"/","MirGeneDB-mature_high_conf.txt"), header = F)
}

if(opt$miRdb == "miRBase"){
